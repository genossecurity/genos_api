...
def infer_tier2(engine: GenosEngine, processed_cmd: str, topk: int = 3) -> Dict[str, object]:
    t2_text, _ = engine._build_variant_a_text(processed_cmd)
    proba = engine.t2.predict_proba([t2_text])[0]
    top_idxs_np = proba.argsort()[-topk:][::-1]
    top_codes = [engine._tfidf_idx_to_label.get(int(engine.t2.classes_[i]), "?") for i in top_idxs_np]
    top_probs = [float(proba[i]) for i in top_idxs_np]

    return {
        "top1_idx": int(engine.t2.classes_[top_idxs_np[0]]),
        "topk_idxs": np.array([int(engine.t2.classes_[i]) for i in top_idxs_np]),
...
def main() -> None:
    ...
    print(
        f"{'TF-IDF Char RF Specialist':<32} | "
    ...
    if tier2_expanded_res is not None:
        print(
            f"{'TF-IDF Char RF (147-class expanded)':<32} | "

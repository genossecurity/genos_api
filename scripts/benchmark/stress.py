...
def run_offline_audit(sample_size=1000):
    ...
    b_df = pd.read_csv(os.path.join(BASE_DIR, "data", "benign_final.csv")).sample(sample_size)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Unified Genos benchmark runner")
    parser.add_argument(
        "--mode",
